...
public class GoalProgressProcessor implements ItemProcessor<List<Goal>, List<DailyGoalProgress>> {
	...
	public List<DailyGoalProgress> process(List<Goal> goals) throws Exception {
		...
		for (Goal goal : goals) {
			try {
				DailyGoalProgress dailyProgress = calculateGoalProgress(goal, allActions, allMemberProducts,
					allAccounts);
				...
				Integer goalRate = (int)((dailyProgress.getIsaProgress() + dailyProgress.getDepositProgress()) * 100);
				log.info("goal_prgress_rate: {}", goalRate);
				if (goalRate == 30 || goalRate == 60 || goalRate == 90 || goalRate == 100) {
					GoalMsg goalMsg = new GoalMsg(goal.getMemberId(), goal.getGoalName(), goalRate, 1);
					log.info("goalMsg: {}", goalMsg);
					rabbitTemplate.convertAndSend(goalMsg);
				}
	...
	private DailyGoalProgress calculateGoalProgress(Goal goal, List<Action> allActions,
		...
		BigDecimal isaTargetAmount = targetAmount.multiply(BigDecimal.valueOf(goal.getIsaRate()))
			.divide(BigDecimal.valueOf(100), RoundingMode.HALF_UP);
		BigDecimal depositTargetAmount = targetAmount.multiply(BigDecimal.valueOf(goal.getDepositRate()))
			.divide(BigDecimal.valueOf(100), RoundingMode.HALF_UP);
