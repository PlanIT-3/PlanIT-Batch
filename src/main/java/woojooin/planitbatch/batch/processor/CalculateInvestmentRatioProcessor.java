...
import woojooin.planitbatch.domain.mapper.ProductMapper;
...
public class CalculateInvestmentRatioProcessor implements ItemProcessor<List<Member>, List<InvestmentRatio>> {
    ...
    private InvestmentRatio calculateRatio(Long memberId, List<MemberProduct> memberProducts, Map<Long, Product> products) {
        ...
        Map<String, Long> investTypeCounts = memberProducts.stream()
            ...
            .filter(product -> {
                boolean isValid = product != null && product.getInvestmentType() != null;
                if (!isValid) {
            ...
            .collect(Collectors.groupingBy(
                product -> {
                    String investType = product.getInvestmentType();
